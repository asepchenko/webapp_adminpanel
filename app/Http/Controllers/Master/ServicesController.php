...
class ServicesController extends Controller
{
    public function index(Request $request)
    {
        ...
        $servicegroups = $response->data;
        return view('master.services.index',compact('servicegroups'));
    }
    ...
    {
        $api_url = config('api.api_url')."/services/".$id;
        $response = GuzzleHelper::get($request, $api_url);
        return response()->json($response->data);
    }
    ...
    {
        $api_url = config('api.api_url')."/services/".$request->id;
