      ...
      <div class="container pd-x-0 pd-lg-x-10 pd-xl-x-0">
        <div class="d-sm-flex align-items-center justify-content-between">
          <div>
            ...
            <h4 class="mg-b-0">List Order STT</h4>
          </div>
          <div class="mg-t-20 mg-sm-t-0">
            <button class="btn btn-sm btn-brand-02" type="button" id="btn-create"><i class="fas fa-plus"></i> Buat Manifest</button>
          </div>
        </div>
      </div><!-- container -->
    </div><!-- content -->
...
$(function(){
  ...
  $("#btn-create").on("click", function() {
        ...
        if (!ids || !ids.length) {
            ...
            //return false;
            console.log(ids);
            var r = confirm("Anda yakin ingin membuat manifest dengan data STT ini ?");
            if (r == true) {
                event.preventDefault();
                var spinner = $('#loader');
                spinner.show();
                ...
                $.ajax({
                    ...
                    success: function (result) {
                      alert("Berhasil Membuat Manifest dengan nomor : "+result.data);
                      window.location.href= "{{ url('warehouse/manifest') }}/"+result.data;
                    },
  ...
  var table = $('#table_data').DataTable({
    ...
    //serverSide: true,
    ajax: {
      url: '{{ url("transaction/orders/list") }}',
      type: 'GET',
    },
    columnDefs: [ 
      {
