...
<div class="content content-fixed bd-b">
      <div class="container pd-x-0 pd-lg-x-10 pd-xl-x-0">
        <div class="d-sm-flex align-items-center justify-content-between">
          <div>
            ...
            <h4 class="mg-b-0">List Jenis Pembayaran</h4>
          </div>
          @can('master_payment_type_create')
      ...
      <div class="container pd-x-0 pd-lg-x-10 pd-xl-x-0">
        ...
        <div data-label="Example" class="df-example demo-table">
          <table id="table_data" class="table">
            <thead>
                <tr>
                    <th class="wd-60p">Nama Pembayaran</th>
                    <!--<th class="wd-20p">Termin (Bulan)</th>-->
    ...
    @include('master.payment-types.create-modal')
    @include('master.payment-types.delete-modal')
...
<script>
  ...
  function editData(data){
    ...
    loader.show();
    $.ajax({
		  url: '{{ url("master/payment-types") }}'+'/'+data,
      method:"GET",
		  success:function(data){
        loader.hide();
        $('#id').val(data.id);
        $('#payment_name').val(data.payment_name);
        $('#termin_month').val(data.termin_month);
        $('#action').val('edit');
        $('#modal_title').text('Ubah Jenis Pembayaran');
...
$(function(){
  ...
  $('#btn-add-new').click(function(){
    $('#modal_title').text('Tambah Jenis Pembayaran');
    $('#id').val('');
    $('#payment_name').val('');
    $('#termin_month').val('');
    $('#action').val('add');
  ...
  $('#table_data').DataTable({
    ...
    //serverSide: true,
    ajax: {
      url: '{{ url("master/payment-types/datatable") }}',
      type: 'GET',
    },
    columns: [
      { data: 'payment_name', name: 'payment_name' },
      //{ data: 'termin_month', name: 'termin_month' },
  ...
  $("#frm-add").submit(function (e) {
    e.preventDefault();
    if($('#action').val() == "add"){
      var action = "{{ route('payment-types.store') }}";
    }else{
      var action = "{{ route('payment-types.update') }}";
    }
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        window.location.href = "{{ url('master/payment-types') }}";
      },
