...
<div class="content content-fixed bd-b">
          ...
          <div>
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                ...
                <li class="breadcrumb-item active" aria-current="page">Lokasi (Tarif Reguler)</li>
              </ol>
            </nav>
            <h4 class="mg-b-0">List Lokasi (Tarif Reguler)</h4>
          </div>
          @can('master_regular_price_create')
    ...
    @include('master.locations.create-modal')
    @include('master.locations.delete-modal')
...
<script>
  ...
  function editData(data){
    window.location.href = '{{ url("master/locations") }}'+'/'+data;
  }
...
$(function(){
  ...
  $('#table_data').DataTable({
    ...
    //serverSide: true,
    ajax: {
      url: '{{ url("master/locations/datatable") }}',
      type: 'GET',
    },
    columns: [
      ...
      { data: 'publish_price', name: 'publish_price' },
      { data: 'sales_bottom', name: 'sales_bottom' },
      {
        data: 'id',
        orderable: false,
  ...
  $("#frm-add").submit(function (e) {
    e.preventDefault();
    if($('#action').val() == "add"){
      var action = "{{ route('locations.store') }}";
    }else{
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        window.location.href = "{{ url('master/locations') }}/" + data.data;
      },
