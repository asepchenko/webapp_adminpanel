...
<div class="content content-fixed bd-b">
      <div class="container pd-x-0 pd-lg-x-10 pd-xl-x-0">
        <div class="d-sm-flex align-items-center justify-content-between">
          <div>
            ...
            <h4 class="mg-b-0">List Cabang</h4>
          </div>
          @can('master_branchs_create')
          <div class="mg-t-20 mg-sm-t-0">
            <button type="button" class="btn btn-sm btn-brand-02" id="btn-add-new"><i class="fas fa-plus"></i> Tambah</button>
          </div>
          @endcan
        </div>
      </div><!-- container -->
    ...
    @include('master.branchs.create-modal')
    @include('master.branchs.delete-modal')
...
$(function(){
  ...
  $('#btn-add-new').click(function(){
    $('#modal_title').text('Tambah Cabang');
    $('#id').val('');
    $('#branch_code').val('');
    $('#branch_name').val('');
    $('#city_id').val('');
  ...
  $('#table_data').DataTable({
    ...
    //serverSide: true,
    ajax: {
      url: '{{ url("master/branchs/datatable") }}',
      type: 'GET',
    },
    columns: [
        ...
        mRender: function (data, type, obj) {
          if (data !== "") {
            ...
            data.forEach(function (item, index) {
              ...
              empolyees += '<span class="badge badge-pill badge-primary"><b>'+item.name+'</b></span>&nbsp;';
            });
            return empolyees; //orders.slice(0, -1);
  ...
  $("#frm-add").submit(function (e) {
    e.preventDefault();
    if($('#action').val() == "add"){
      var action = "{{ route('branchs.store') }}";
    }else{
      var action = "{{ route('branchs.update') }}";
    }
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        window.location.href = "{{ url('master/branchs') }}";
      },
