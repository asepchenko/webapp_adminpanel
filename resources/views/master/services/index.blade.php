...
<div class="content content-fixed bd-b">
      ...
      <div class="container pd-x-0 pd-lg-x-10 pd-xl-x-0">
        ...
        <div data-label="Example" class="df-example demo-table">
          <table id="table_data" class="table">
            <thead>
                <tr>
                    <th class="wd-20p">Nama Layanan</th>
                    <th class="wd-20p">Grup</th>
    ...
    @include('master.services.create-modal')
    @include('master.services.delete-modal')
...
$(function(){
  ...
  $('#table_data').DataTable({
    ...
    //serverSide: true,
    ajax: {
      url: '{{ url("master/services/datatable") }}',
      type: 'GET',
    },
    columns: [
  ...
  $("#frm-add").submit(function (e) {
    e.preventDefault();
    if($('#action').val() == "add"){
      var action = "{{ route('services.store') }}";
    }else{
      var action = "{{ route('services.update') }}";
    }
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        window.location.href = "{{ url('master/services') }}";
      },
