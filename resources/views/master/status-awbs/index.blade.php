...
<div class="content content-fixed bd-b">
          ...
          <div>
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                ...
                <li class="breadcrumb-item active" aria-current="page">Status AWB</li>
              </ol>
            </nav>
            <h4 class="mg-b-0">List Status AWB</h4>
          </div>
    ...
    @include('master.status-awbs.create-modal')
    @include('master.status-awbs.delete-modal')
...
<script>
  ...
  function editData(data){
    ...
    loader.show();
    $.ajax({
		  url: '{{ url("master/status-awbs") }}'+'/'+data,
      method:"GET",
        ...
        $('#action').val('edit');
        $('#modal_title').text('Ubah Status AWB');
...
$(function(){
  ...
  $('#btn-add-new').click(function(){
    $('#modal_title').text('Tambah Status AWB');
    $('#id').val('');
    $('#status_name').val('');
    $('#action').val('add');
		$('#frm-modal').modal('show');
  ...
  $('#table_data').DataTable({
    ...
    serverSide: true,
    ajax: {
      url: '{{ url("master/status-awbs/datatable") }}',
      type: 'GET',
    },
    columns: [
  ...
  $("#frm-add").submit(function (e) {
    e.preventDefault();
    if($('#action').val() == "add"){
      var action = "{{ route('status-awbs.store') }}";
    }else{
      var action = "{{ route('status-awbs.update') }}";
    }
