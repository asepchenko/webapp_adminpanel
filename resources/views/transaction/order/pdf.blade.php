...
<body>
	...
	<div class="row">
		...
		<div class="column"">
			<p>
                @php 
                echo DNS1D::getBarcodeHTML($awb_no, 'C128');
                @endphp
            </p>
            <!--<p>{{ $awb_no }}</p>-->
		</div>
	...
	<table style="width:100%">
		<tr>
			...
			<td>Tanggal</td>
			<td>{{ (new \App\Helpers\DateHelper)->getMonthName($order->pickup_date) }}</td>
		</tr>
	</table>
	<br>
	<table class="table-border">
		<thead>
		  <tr>
			<th>Pengirim</th>
			<th>{{ $order->customers->customer_name }}</th>
			<th colspan="2"><b>ISI BARANG TIDAK DIPERIKSA</b></th>
			<th colspan="2"><b>No AWB</b></th>
			<th colspan="2">{{ $order->awb_no }}</th>
		  </tr>
		</thead>
		<tbody>
		  <tr>
			<td colspan="2" rowspan="4">&nbsp;Alamat : {{ $order->customers->address }}</td>
		  ...
		  <tr>
			<td colspan="4" rowspan="2">&nbsp;{{ $order->contains }} </td>
			<td rowspan="2">&nbsp;{{ $order->origins->city_code }}</td>
			<td rowspan="2">&nbsp;{{ $order->destinations->city_code }}</td>
		  </tr>
		  <tr>
		  </tr>
		  <tr>
			<td colspan="2">&nbsp;Telp : {{ $order->customers->phone_number }}</td>
		  ...
		  <tr>
			...
			<td rowspan="2">&nbsp;{{ $order->customers->customer_name }} - {{ $order->customer_branchs->branch_name }}</td>
		  ...
		  <tr>
			<td colspan="2" rowspan="3">&nbsp;Alamat : {{ $order->customer_branchs->address }}</td>
		  ...
		  <tr>
			<th rowspan="3">{{ $order->total_colly }}</th>
			<th rowspan="3">{{ $order->total_kg }}</th>
			<th rowspan="3"></th>
			<td>&nbsp;Inv</td>
			<td>&nbsp;</td>
			<th></th>
		  ...
		  <tr>
			<td colspan="8" rowspan="2" height="30px">&nbsp;Keterangan : {{ $order->description }}</td>
		  </tr>
		  <tr></tr>
		  <!-- start ttd -->
			<tr>
