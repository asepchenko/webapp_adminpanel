          ...
          <div>
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                ...
                <li class="breadcrumb-item active" aria-current="page">Harga Trucking Customer</li>
              </ol>
            </nav>
            <h4 class="mg-b-0">List Pending Harga Trucking Customer</h4>
          </div>
...
<script>
  function approveData(data){
    if (confirm("Anda yakin ingin melakukan approve pada data ini?") == true) {
      ...
      $.ajax({
        url: '{{ url("approval/customer-trucking-prices/approve") }}',
        type: "POST",
        headers: {
        ...
        success:function(data){
          ...
          window.location.href = "{{ url('approval/customer-trucking-prices') }}";
        },
        error: function(data){
          loader.hide();
          alert(data);
  ...
  $("#btn-create").on("click", function() {
        ...
        if (!ids || !ids.length) {
            ...
            if (r == true) {
                ...
                form_data.append('_token', '{{csrf_token()}}');
                $.ajax({
                    url: "{{ url('approval/customer-trucking-prices/mass-approve') }}",
                    method:"POST",
                    data:form_data,
                    contentType: false,
                    processData: false,
    ...
    //serverSide: true,
    ajax: {
      url: '{{ url("approval/customer-trucking-prices/datatable") }}',
      type: 'GET',
    },
    columnDefs: [ 
      {
